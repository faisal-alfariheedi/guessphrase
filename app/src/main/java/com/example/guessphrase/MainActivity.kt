...
class MainActivity : AppCompatActivity() {
                    ...
                    false -> {
                        if (limitl > 0) {
                            if (inin.text.toString().count() == 1) {
                                ...
                                if (inin.text.toString()
                                        .isEmpty()
                                ) showAlertDialog("it can`t be empty", 0)
                                else {
    ...
    fun showAlertDialog(title: String, a: Int) {
        ...
        if (a == 1) dialogBuilder.setPositiveButton(
            "Yes",
            DialogInterface.OnClickListener { dialog, id ->
                ...
                overridePendingTransition(
                    0,
                    0
                );//this.recreate();input.clear();rv.adapter = RVAdapter(input);prq=prag[(0..(prag.size-1)).random()]
            })
        else dialogBuilder.setPositiveButton("OK", DialogInterface.OnClickListener { dialog, id ->
            null
        })
